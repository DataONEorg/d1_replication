...
public class ReplicationManager implements ItemListener<MNReplicationTask> {
    ...
    public int createAndQueueTasks(Identifier pid) throws ServiceFailure, NotImplemented,
        ...
        List<NodeReference> preferredList = replicationPolicy.getPreferredMemberNodeList();
        List<NodeReference> blockedList = replicationPolicy.getBlockedMemberNodeList();

        log.info("Removing blocked nodes from the potential replication list for " + pid.getValue());
        ...
        if (preferredList != null && !preferredList.isEmpty()) {
            ...
            for (int i = preferredList.size() - 1; i >= 0; i--) {
                ...
                if (potentialNodeList.contains(preferredNode)) {
                    potentialNodeList.remove(preferredNode); // remove for
                                                             // reordering
                    potentialNodeList.add(0, preferredNode); // to the top of
                                                             // the list
